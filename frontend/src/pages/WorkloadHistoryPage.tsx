...
export function WorkloadHistoryPage() {
  ...
  const [items, setItems] = useState<WorkloadHistory[]>([])
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [history, historyAggregate, historyTrends] = await Promise.all([
        listWorkloadHistory(query),
      ...
      setAggregate(null)
      setTrends(null)
      setError(getErrorMessage(err, 'Failed to load workload history.'))
    } finally {
      setLoading(false)
    }
    ...
    <Stack spacing={3}>
      <Box>
        ...
        <Typography color="text.secondary">
          Immutable snapshots of completed Workload Engine calculations (US-107).
        </Typography>
      </Box>
      ...
      <Paper sx={{ p: 2 }}>
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} useFlexGap flexWrap="wrap">
          ...
          <Button component={RouterLink} to="/workload/history/aggregate" variant="outlined">
            Aggregation View
          </Button>
        </Stack>
      </Paper>
      ...
      <Paper sx={{ p: 2 }}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems={{ sm: 'center' }}>
          ...
          <Button
            ...
            to={`/workload/history/compare?leftHistoryId=${encodeURIComponent(leftId)}&rightHistoryId=${encodeURIComponent(rightId)}`}
            variant="outlined"
            ...
            <Table size="small">
              ...
              <TableBody>
                {trends.points.map((point, index) => (
                  <TableRow key={`${point.calculationDate}-${index}`}>
                    <TableCell>{new Date(point.calculationDate).toLocaleString()}</TableCell>
                    <TableCell>
                      {point.periodStart} → {point.periodEnd}
                    </TableCell>
                    <TableCell align="right">{point.allocatedHours}</TableCell>
                    <TableCell align="right">{point.capacityHours}</TableCell>
                    <TableCell align="right">{point.workloadPercentage}%</TableCell>
                    <TableCell>{point.calculationVersion}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
            ...
            <TableBody>
              {items.map((item) => (
                <TableRow key={item.historyId} hover>
                  ...
                  <TableCell>
                    {item.executionResourceName || item.executionResourceId}
                  </TableCell>
                  <TableCell>
                    {item.periodStart} → {item.periodEnd}
                  </TableCell>
                  <TableCell align="right">{item.allocatedHours}</TableCell>
                  <TableCell align="right">{item.capacityHours}</TableCell>
                  <TableCell align="right">{item.workloadPercentage}%</TableCell>
                  <TableCell>{item.calculationVersion}</TableCell>
                  <TableCell align="right">
                    <Button
                      component={RouterLink}
                      to={`/workload/history/${item.historyId}`}
                      size="small"
                    >
                      Detail
                    </Button>
                ...
                <TableRow>
                  <TableCell colSpan={8}>
                    <Typography color="text.secondary" py={2}>
                      No historical workload records match the current filters.
                    </Typography>
                  </TableCell>
                </TableRow>
              )}
