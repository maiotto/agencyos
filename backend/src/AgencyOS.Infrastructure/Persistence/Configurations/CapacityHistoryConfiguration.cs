...
public class CapacityHistoryConfiguration : IEntityTypeConfiguration<CapacityHistory>
{
    public void Configure(EntityTypeBuilder<CapacityHistory> builder)
    {
        builder.ToTable("capacity_history");
        ...
        builder.Property(history => history.ConfiguredHours).HasColumnName("configured_hours").HasPrecision(12, 2).IsRequired();
        builder.Property(history => history.AvailableHours).HasColumnName("available_hours").HasPrecision(12, 2).IsRequired();
        ...
        builder.Property(history => history.UtilizationPercentage).HasColumnName("utilization_percentage").HasPrecision(8, 2).IsRequired();
        ...
        builder.HasIndex(history => history.ExecutionResourceId)
            .HasDatabaseName("idx_capacity_history_execution_resource_id");
        builder.HasIndex(history => history.CompanyId)
            .HasDatabaseName("idx_capacity_history_company_id");
        builder.HasIndex(history => new { history.PeriodStart, history.PeriodEnd })
            .HasDatabaseName("idx_capacity_history_period");
        builder.HasIndex(history => history.CalculationVersion)
            .HasDatabaseName("idx_capacity_history_calculation_version");
    }
}
