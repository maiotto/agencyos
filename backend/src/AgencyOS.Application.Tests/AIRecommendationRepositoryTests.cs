...
public class AIRecommendationRepositoryTests
{
    [Fact]
    public async Task AddGetAndVersioning_Work()
    {
        ...
        var repository = new AIRecommendationRepository(context);
        var generator = new AIRecommendationGenerationService();

        var v1 = generator.Generate(recommendation, 1, "planner", DateTimeOffset.UtcNow);
        await repository.AddAsync(v1);

        var next = await repository.GetNextGenerationVersionAsync(recommendation.Id);
        var v2 = generator.Generate(recommendation, next, "planner", DateTimeOffset.UtcNow.AddMinutes(1));
    ...
    [Fact]
    public async Task Query_FiltersByStatusAndConfidence()
    {
        await using var context = CreateContext();
        var recommendation = await SeedRecommendationAsync(context);
        var repository = new AIRecommendationRepository(context);
        var generator = new AIRecommendationGenerationService();
        var item = generator.Generate(recommendation, 1, "planner", DateTimeOffset.UtcNow);
        await repository.AddAsync(item);

        var results = await repository.QueryAsync(new AIRecommendationQueryParameters
        {
            Status = AIRecommendationStatus.Active,
            MinConfidenceScore = 30m,
            Search = "advisory"
        });
