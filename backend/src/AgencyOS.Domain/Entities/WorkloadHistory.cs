...
/// <summary>
/// Immutable Workload History aggregate (US-107 / BR-701..BR-709).
...
public class WorkloadHistory
{
    ...
    public decimal WorkloadPercentage { get; private set; }
    ...
    /// <summary>
    /// JSON snapshot of operational inputs used during calculation (BR-709).
    /// </summary>
    public string OperationalInputsJson { get; private set; } = string.Empty;

    public DateTimeOffset CreatedAt { get; private set; }

    private WorkloadHistory()
    {
    }

    public static WorkloadHistory Create(
        ...
        DateTimeOffset calculationDate)
    {
        ...
        {
            throw new InvalidOperationException("Execution Resource is mandatory for workload history.");
        }

        if (companyId == Guid.Empty)
        {
            throw new InvalidOperationException("CompanyId is mandatory for workload history.");
        }
